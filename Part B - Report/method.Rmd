...
Categorical features that were produced from one hot encoding (Table 2):

*	Gender (Female, Male)

*	Accident Type (Accident_Type_DescCollision.with.a.fixed.object, ...Struck.animal, ...Struck.Pedestrian, ...Vehicle.overturned&no.collision)

*	Road surface type and condition (Road_Surface_Type_DescUnpaved, Surface_Cond_DescDry, …Icy, ...Muddy, ...Snowy, ...Wet)

*	Weather and light conditions (Atmosph_Cond_DescClear, ...Fog, ...Raining, ...Smoke, ...Strong.winds, Light_Condition_DescDark.No.street.lights, ...Street.lights.off, ConditionsOvercast, ...Rain, ...Rain&Overcast)

*	Age group (Age_Group16.17, ...17.21, ...70)

*	Weekend (Day_Week_DescriptionSaturday, ...Sunday)

*	Cylinders of car (NO_OF_CYLINDERS_4, ...6, ...8, ...12)

Numerical features:

*	Standardized variables from traffic accidents data (NO_OF_VEHICLES, NO_PERSONS, SPEED_ZONE, TOTAL_NO_OCCUPANTS, LIGHT_CONDITION)

*	Standardized variables from weather data (CloudCover, RelativeHumidity, Precipiation)

*	Original variables from traffic accidents data (VEHICLE_YEAR_MANUF)

*	Original variables from weather data (WindSpeed, Temperature, DewPoint)
...
\begin{center}
Table 3. Datasets for building the model
\end{center}
...
In the binary-class problem, the confusion matrix (Table 4) displays the results of correctly and incorrectly perceived cases of each class. The confusion matrix is used to assess the classification performance of both common and uncommon classes.
...
![Cross Validation ROC Curve](ROC_3.png){width=300px}